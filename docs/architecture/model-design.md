...
| 类型 | 描述 | 计算方法 |
|------|------|---------|
| `import` | 导入/依赖关系 | 解析代码 import/require 语句 |
| `reference` | 引用关系 | 查找文件名引用 |
| `similar_name` | 命名相似 | 编辑距离 < 阈值 |
| `same_dir` | 同目录 | 父目录相同 |
| `same_ext` | 同类型 | 扩展名相同 |
| `created_together` | 同时创建 | 创建时间差 < 1分钟 |
...
| 方案 | 优点 | 缺点 | 适用场景 |
|------|------|------|---------|
| 本地轻量向量库 | 隐私保护、低延迟 | 准确性有限 | 文件名/路径搜索 |
| Chroma | 开源、易用 | 需额外安装 | 中等规模数据 |
| Milvus | 功能强大 | 资源消耗大 | 大规模数据 |
